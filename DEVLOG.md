...
### [2026-07-02 00:10] - T-043 - Full Stack Developer
- **Summary**: Synced local database, seeded dummy data, and connected the dashboard to the backend API.
- **Technical Decisions**: Created `maintenanceActualModel.js` and `maintenanceAbnormalLogModel.js`. Modified `maintenanceScheduleModel.js` and `maintenanceLogSheetModel.js` models. Updated imports, instantiations, associations, and exports in `be/src/models/index.js`.
- **Blockers**: None.
- **Next Step**: Implement target generation logic in `be/src/modules/cmms/maintenanceSchedule/checkboxGenerator.js` (T-004).
...
- **Summary**: Synced local database, seeded dummy data, and connected the dashboard to the backend API.
- **Technical Decisions**:
  1. **Database Adjustments**: Created and ran `be/scratch/sync_maintenance.js` to automatically map model schemas (`YearlyStandardMaintenance`, `StandardMaintenance`, `MaintenanceSchedule`, `MaintenanceLogSheet`, etc.) into the SQL database tables using Sequelize `.sync()`.
  2. **Data Seeding**: Executed `seed_dummy.js` to populate mock `Company`, `Users`, `Assets`, `Maintenance Schedules`, `Actuals`, and `Log Sheets` to test maintenance flow.
  3. **Backend Dashboard API**: Added `getDashboardSummary` inside `dashboardController.js` and registered `/api/dashboard/summary` in `dashboardRoute.js` and `index.js`. Maps `AssetBudget` and joined `MaintenanceLogSheet` -> `MaintenanceSchedule` -> `Asset` -> `User` data.
  4. **Frontend Integration**: Updated `DashboardPage.jsx` to fetch data from the newly created dashboard endpoint. Replaced `mockAsset` and `mockMaintLogs` with dynamic state variables bound to API response, maintaining the premium template layout while presenting actual functional data.
- **Blockers**: MSSQL constraint limits required careful `sync()` usage over `alter: true`.
- **Next Step**: Polish data presentation or integrate Operational Budgets if tables become available.

### [2026-07-02 00:15] - T-044 - Full Stack Developer
- **Summary**: Connected the Summary Center template to dynamic backend statistics.
- **Technical Decisions**:
  1. **Backend API**: Created `getFullSummary` endpoint in `dashboardController.js` (mapped to `/api/dashboard/full-summary`). The endpoint executes aggregate `COUNT` queries across `Asset`, `AssetBudget`, `MaintenanceLogSheet`, and `MaintenanceActual` tables to generate holistic statistics.
  2. **Frontend UI**: Refactored `SummaryPage.jsx` to fetch data from the new endpoint on mount. Bound statistic cards (Asset Totals, Budget Progress, Maintenance Logsheets) and the recent logsheet data table to the live React state, replacing static mock values.
- **Blockers**: None.
- **Next Step**: Await user feedback on further data mapping for financial or detailed table aggregates.

### [2026-07-02 00:34] - T-045 - Full Stack Developer
- **Summary**: Refactored standard maintenance periodic definitions, added Daily mapping, and implemented frontend auto-mapping with strict `isoWeek` synchronization.
- **Technical Decisions**:
  1. **Option Alignment**: Standardized `PERIODIK_OPTIONS` across `PreviewGrid.jsx`, `FormModal.jsx`, and `EditableCell.jsx` to strictly match user requests (`DAILY`, `1X/MINGGU`, `2X/MINGGU`, `1X/BULAN`, `3X/BULAN`, `6X/BULAN`, `1X/TAHUN`).
  2. **Frontend Auto-Mapping**: Injected `generatePreviewDates` into `PreviewGrid.jsx` to automatically pre-fill calendar checkboxes during initial load, dropdown changes, or manual row additions.
  3. **Backend Support**: Updated `checkboxGenerator.js` (`parsePeriodik` and `generateCheckboxDates`) to handle the new `daily` and `biweekly` strings.
  4. **Validation Synchronization**: Discovered and resolved an `isoWeek` boundary edge case for `1X/MINGGU` and `2X/MINGGU` schedules. Changed both the frontend generator and backend generator to iterate strictly from `w = 1` to `totalIsoWeeks` using DayJS `isoWeek(w)`. This guarantees that one plan is placed within every verified week block, completely eliminating "Minggu yang kurang: 1" validation desyncs around January 1st cross-year borders.
- **Blockers**: None.
